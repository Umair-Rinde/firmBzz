...
import { Label } from "@/components/ui/custom/custom-label";
...
const VendorOrderAddPage = () => {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: (data: any) =>
      axios.post(`/firm/${firmId}/vendor-orders/`, data),
    onSuccess: () => {
  ...
  const handleSubmit = (values: any) => {
    const payload = {
      ...
      total_amount: values.items.reduce(
        (acc: number, item: any) =>
          acc + item.quantity_ordered * item.cost_price_per_unit,
        0,
      ),
    ...
    <div className="dashboard-page-offset pb-20 max-w-full min-w-0">
      <div className="flex items-center gap-4 mb-6">
        ...
        <AppBar
          ...
          subTitle="Place a new order with your vendor."
          extraButtons={
            <CustomButton
              variant="outline"
              size="icon"
      ...
      >
        {({ values, setFieldValue }) => (
          ...
          (
            <Form className="space-y-8 bg-white md:p-8 rounded-xl border border-gray-100 shadow-sm">
                        ...
                        <tbody>
                          {values.items.map((item, index) => (
                            <tr key={index}>
                              ...
                              <td className="p-2 border">
                                <CustomInput
                                  name={`items.${index}.cost_price_per_unit`}
                                  type="number"
                                  className="!h-11"
                                />
                              </td>
                              <td className="p-2 border">
                                <CustomInput
                                  name={`items.${index}.selling_price_super_seller`}
                                  type="number"
                                  className="!h-11"
                                />
                              </td>
                              <td className="p-2 border">
                                <CustomInput
                                  name={`items.${index}.selling_price_distributor`}
                                  type="number"
                                  className="!h-11"
                                />
                              </td>
                  ...
                  <p className="text-3xl font-bold text-teal-600">
                    ...
                    {values.items
                      .reduce(
                        (acc, item) =>
                          acc +
                          item.quantity_ordered * item.cost_price_per_unit,
                        0,
                      )
                      .toFixed(2)}
                  </p>
