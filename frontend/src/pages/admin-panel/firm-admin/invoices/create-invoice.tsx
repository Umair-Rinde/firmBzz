  ...
  const fetchPricing = async (customer: any, items: any[]) => {
    ...
    try {
      const res = await axios.post(
        `/firm/${firmId}/invoices/preview-pricing/`,
        {
                          ...
                          <table className="w-full border-collapse">
                            ...
                            <tbody>
                              {values.items.map((item, index) => {
                                ...
                                const itemTouched = (touched.items as any)?.[
                                  index
                                ];
                                return (
                                  <tr
                                  ...
                                  >
                                    <td className="p-3">
                                      <Autocomplete
                                        ...
                                        onChange={(p) =>
                                          handleProductSelection(p, index)
                                        }
                                        className="!w-full"
                                      />
                                    </td>
                                    <td className="p-3">
                                      <CustomInput
                                        ...
                                        onChange={(e: any) => {
                                          ...
                                          const updatedItems = values.items.map(
                                            (item, i) =>
                                              i === index
                                                ? {
                                                    ...item,
              ...
              {(isPreviewing || pricing) && (
                <div className="bg-blue-50 border border-blue-100 rounded-xl p-6 space-y-4">
                  <div className="flex items-center gap-2 text-blue-700 font-semibold text-sm">
                    ...
                    {isPreviewing
                      ? "Calculating pricing..."
                      : "Estimated Pricing Breakdown (FEFO)"}
                  </div>
                  {!isPreviewing && pricing && (
                    <>
                      {pricing.items.map((item: any, idx: number) => (
                        <div
                          key={idx}
                          className="bg-white rounded-lg border border-blue-100 p-4 space-y-2"
                        >
