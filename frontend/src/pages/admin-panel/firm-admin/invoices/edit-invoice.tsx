...
const InvoiceEditPage = () => {
  ...
  const { mutate: updateInvoice, isPending: isUpdating } = useMutation({
    mutationFn: (data: any) =>
      axios.put(`/firm/${firmId}/invoices/${id}/`, data),
    onSuccess: () => {
        ...
        {({ values, setFieldValue }) => {
          ...
          return (
            <Form className="space-y-8 bg-white p-8 rounded-xl border border-gray-100 shadow-sm relative">
              ...
              <div
                className={`grid grid-cols-1 md:grid-cols-2 gap-6 pb-6 border-b border-gray-100 ${!isEditable ? "opacity-80" : ""}`}
              >
                <div>
                  ...
                  <Autocomplete
                    ...
                    getOptionLabel={(c: any) =>
                      `${c.business_name} (${c.customer_type === "SUPER_SELLER" ? "Super Seller" : "Distributor"})`
                    }
                        ...
                        <table className="w-full border-collapse">
                          <thead>
                            <tr className="bg-gray-50 text-left text-sm font-medium text-gray-500">
                              <th className="p-3 border-b min-w-[250px]">
                                Product *
                              </th>
                              <th className="p-3 border-b w-32">Batch</th>
                              <th className="p-3 border-b w-32">Quantity *</th>
                              <th className="p-3 border-b w-40">
                                Allocated Rate (₹)
                              </th>
                              <th className="p-3 border-b w-40 text-right">
                                Amount (₹)
                              </th>
                              {isEditable && (
                                  ...
                                  <td className="p-3">
                                    <Autocomplete
                                      ...
                                      getOptionLabel={(p: any) =>
                                        `${p.name} (Stock: ${p.available_quantity || 0})`
                                      }
                      ...
                      {isEditable && (
                        <div className="flex justify-between items-center">
                          ...
                          <p className="text-xs text-blue-500 max-w-xs text-right">
                            Saving changes will re-calculate batch pricing
                            automatically.
                          </p>
                        </div>
                      )}
                  ...
                  {isEditable && (
                    <div className="flex gap-4 pt-4 relative z-20">
                      <CustomButton
                        ...
                        disabled={
                          isUpdating || isApproving || values.items.length === 0
                        }
